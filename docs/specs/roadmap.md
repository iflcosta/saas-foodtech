...
**Fase 4 em andamento — fatia 4b.1 entregue.** Fundação (auth + tenant + Postgres)
mais cardápio admin completo (categorias, produtos, grupos de modificadores,
modificadores, attach N:N produto↔grupo) e cardápio público (`GET /v1/menu/:slug`
com tree aninhado de 6 níveis filtrando `deleted_at IS NULL` em todos os nós).
**40 testes verdes** (12 unit + 28 integração contra Postgres real, incluindo
isolamento cross-tenant em cada CRUD). Próximo: **fatia 4b.2** — ingestão de pedido
(`POST /v1/menu/:slug/orders` + `POST /v1/orders` com recálculo autoritativo de
preço e pizza fracionada). Ver §4.1.
...
| 4 | Implementação da V1.0 | Em andamento — 4a + 4b.1 entregues |
...
Fatia 4b.1 (cardápio admin + leitura pública) entregue. Próxima frente: 4b.2 — ingestão.
...
| Fundação | Validadores Zod (`packages/shared`) | 11 domínios — cardápio cobre o necessário para 4b.1 |
| Fundação | Auth JWT + middleware de tenant | Implementado (`POST /v1/auth/login`, `app.authenticate`, `app.requireRole`) |
| RF-1 | Ingestão de pedidos (cardápio, WhatsApp, triagem) | Não iniciado — próxima fatia (4b.2) |
...
| 4b.2 — Ingestão de pedido | **Próxima** | `POST /v1/menu/:slug/orders` (público) + `POST /v1/orders` (manual, operator) + recálculo autoritativo de `unit_price` / `subtotal` / `total` no servidor (RNF §12) + pizza fracionada (ADR-Q9) + combos rejeitados se preço zerar | RF-1 ingestão, RF-3.3 | "Pedido aparece em `orders` com status `pending`" |
...
4. **Rate-limit por IP no cardápio público.** `GET /v1/menu/:slug` exposto sem auth e o
   `api-contracts.md §5` exige rate-limit. Implementação adiada (rota é lê-only contra
   índices, baixo risco no MVP); tratar com `@fastify/rate-limit` antes do lançamento
   ou junto da fatia 4b.2 (que adiciona `POST /v1/menu/:slug/orders` — escrita pública
   que SIM precisa de proteção).
